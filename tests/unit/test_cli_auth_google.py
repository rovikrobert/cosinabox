...
def test_auth_google_prints_refresh_token() -> None:
    ...
    with patch.dict(os.environ, {"GOOGLE_OAUTH_CLIENT_ID": "cid", "GOOGLE_OAUTH_CLIENT_SECRET": "sec"}, clear=True):
        with patch("cosinabox.cli.auth_google.InstalledAppFlow.from_client_config", return_value=fake_flow):
            runner = CliRunner()
