...
class IdentityStep(Step):
    name = "identity"
    def prompt(self) -> str:
        return ("Step 1/10 — Identity. Tell me your name, role, company, and timezone "
...
class StakesStep(Step):
    name = "stakes"
    def prompt(self) -> str:
        return ("Step 2/10 — Stakes. What's the most important thing happening in your work over "
                "the next 6 weeks? A CoS without stakes is a chatbot.")
    def apply(self, answer: str, config_dir: Path) -> None:
        path = config_dir / "personality.md"
        text = path.read_text()
        text = re.sub(r"# Stakes\n.*?(\n#|\Z)", f"# Stakes\n{answer}\n\\1", text, count=1, flags=re.DOTALL)
        path.write_text(text)

class VoiceStep(Step):
    name = "voice"
    def prompt(self) -> str:
        return ("Step 3/10 — Voice. Pick one: blunt / warm / analytical / formal / playful. "
...
class StakeholdersStep(Step):
    name = "stakeholders"
    def prompt(self) -> str:
        return ("Step 4/10 — Top stakeholders. Name your 5 most important people right now. "
    ...
    def apply(self, answer: str, config_dir: Path) -> None:
        ...
        existing = (yaml.safe_load(path.read_text()) if path.exists()
...
class CalendarRealityStep(Step):
    ...
    def prompt(self) -> str:
        return ("Step 5/10 — Calendar reality. What should pre-meeting prep skip? "
                "Common: 'lunch', 'focus block', '1:1'. Comma-separated, or 'none'.")
    def apply(self, answer: str, config_dir: Path) -> None:
        path = config_dir / "jobs.yaml"
        data = (yaml.safe_load(path.read_text()) if path.exists()
                else {"schema_version": 1, "jobs": {}})
        skips = [] if answer.strip().lower() == "none" else [s.strip() for s in answer.split(",") if s.strip()]
...
class JobStagingStep(Step):
    name = "job_staging"
    def prompt(self) -> str:
        return ("Step 6/10 — Job staging. For week 1, I'm enabling only morning_briefing "
...
class OAuthStep(Step):
    name = "oauth"
    def prompt(self) -> str:
        return ("Step 7/10 — API keys + OAuth. Walk through docs/agent/oauth-walkthrough.md "
...
class BudgetStep(Step):
    name = "budget"
    def prompt(self) -> str:
        return ("Step 8/10 — Budget caps. Default daily cap is $15. Want to change? "
...
class FirstSimulationStep(Step):
    ...
    def prompt(self) -> str:
        return ("Step 9/10 — First simulation. I'm about to run "
...
class DeployStep(Step):
    name = "deploy"
    def prompt(self) -> str:
        return ("Step 10/10 — Deploy. I'll walk you through the Railway template + GitHub "
                "repo connect + env var entry. Ready? (yes/no)")
    def apply(self, answer: str, config_dir: Path) -> None:
        pass  # Deployment is external

STEPS: list[Step] = [
    IdentityStep(), StakesStep(), VoiceStep(), StakeholdersStep(),
    CalendarRealityStep(), JobStagingStep(), OAuthStep(), BudgetStep(),
    FirstSimulationStep(), DeployStep(),
]
