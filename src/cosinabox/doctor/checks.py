    ...
    def run(self, *, config_dir: Path, history: dict[str, Any]) -> CheckResult:
        ...
        if len(text) < defaults.DOCTOR_PERSONALITY_MIN_CHARS:
            return CheckResult(self.name, "fail",
                f"personality.md is {len(text)} chars; under threshold of {defaults.DOCTOR_PERSONALITY_MIN_CHARS}. Generic personality = generic briefings.")
...
class CostRunawayCheck(Check):
    ...
    def run(self, *, config_dir: Path, history: dict[str, Any]) -> CheckResult:
        ...
        if hot_days:
            return CheckResult(self.name, "fail",
                f"{len(hot_days)} day(s) above {defaults.DOCTOR_COST_RUNAWAY_RATIO*100:.0f}% of cap (${cap:.0f}/day)")
...
class BriefingDriftCheck(Check):
    ...
    def run(self, *, config_dir: Path, history: dict[str, Any]) -> CheckResult:
        ...
        if "morning_briefing" not in sim_log:
            return CheckResult(self.name, "fail",
                "morning_briefing prompt overridden but never simulated; run `cosinabox simulate morning_briefing`")
...
class SecretInTrackedFileCheck(Check):
    ...
    def run(self, *, config_dir: Path, history: dict[str, Any]) -> CheckResult:
        try:
            ls = subprocess.run(["git", "ls-files"], cwd=config_dir,
...
class OAuthExpiringCheck(Check):
    ...
    def run(self, *, config_dir: Path, history: dict[str, Any]) -> CheckResult:
        ...
        if days_until < defaults.DOCTOR_OAUTH_EXPIRY_WARN_DAYS:
            return CheckResult(self.name, "fail",
                f"Google OAuth refresh token expires in {days_until} days; re-run `cosinabox auth google`")
