...
### Added
- `research_digest` — a weekly, config-driven research digest job. Reads `research.yaml` (groups of orgs/people to track, with per-group priority), collects candidates via Tavily search and RSS, filters feed noise by tracked term, runs a cheap fail-open relevance pass, then makes **one streamed synthesis call**. Signals persist to `research_signals`, which is primary data — the only place a signal survives after delivery — so the store is backed up before each run. Disabled by default; needs `research.yaml`, `TAVILY_API_KEY`, and `pip install 'cosinabox[research]'`, and reports "not configured" rather than raising if any is missing. Ports the core of the maintainer's legacy intel pipeline, including its scars: the priority cap sorts before slicing, and health alerts fire on zero-signal and near-ceiling runs because "the job ran" is not a success signal.
- `call_with_failover_stream` — streamed sibling of `call_with_failover`. A non-streamed call must declare a `max_tokens` it will not exceed, and a long structured response truncates against it silently; this is the fix for two digests corrupted that way.
- `Memory.backup_database` — rolling on-disk SQLite backups using SQLite's own backup API (not a file copy, so a concurrent writer can't produce a torn snapshot). Keeps 7.
...
### Docs
- `docs/plans/2026-08-20-port-intel-core.md` — implementation plan for porting the maintainer's legacy competitive-intel pipeline into the engine as a generic, config-driven `research_digest` job (collect → classify → synthesize → store → notify). 5 milestones, 12 tasks. Publication (GitHub digest, glossary, curriculum) and the scheduler-wide dead-man's switch are explicitly scoped out to their own plans. `docs/active-plan.md` now points here; it had been stale on a plan completed in May.
